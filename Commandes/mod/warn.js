...
module.exports = new Command({
    ...
    async run(bot, message, args, db) {
        ...
        await message.channel.send(`:palm_tree: ${user.tag} a été __warn__ pour \`\`${reason}\`\` !`)
        try {
            await user.send(`:palm_tree: Vous avez été __warn__ sûr \`\`${message.guild.name}**\`\` pour **${reason} !** \n\n __Merci de Faire attention !__ <a:Alerte1:754441316905123994>`)
        } catch (err) {}
