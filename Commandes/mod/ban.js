const {Permissions :{FLAGS :{BAN_MEMBERS}}} = require("discord.js")
...
module.exports = new Command({
    ...
    async run(bot, message, args, db) {
        ...
        try {
            await user.send(`:palm_tree: Vous êtes __banni__ du serveur dû serveur : **${message.guild.name}** pour __**${reason} !**__ <a:EBan1:754441325579075734>`)
        } catch (err) {}
        ...
        await message.reply({content: `:palm_tree: ${user} a été __banni__ pour \`\`${reason}\`\` ! :palm_tree:`}).then(async msg => {
